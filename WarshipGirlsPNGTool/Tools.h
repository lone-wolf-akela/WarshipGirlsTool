 ...
 * 数据块解密
 */
static void DecryptBlock(std::stringstream &ss, const aes_key &key) throw(std::string)
{
	...
	if (contents_size == 0)
	{
		...
		throw s("Error happen in File ")+ std::tr2::sys::path(__FILE__).filename().string()+s(", Function ") + s(__FUNCTION__)
			+s(", Line ")+ std::to_string(__LINE__) + s(" :contents_size is 0!");
	}
	uint32_t real_size = contents_size;
	if (real_size % AES_BLOCK_SIZE) real_size += AES_BLOCK_SIZE - contents_size % AES_BLOCK_SIZE;
